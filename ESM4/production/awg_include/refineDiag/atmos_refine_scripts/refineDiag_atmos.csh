...
set radiation_files = `/bin/ls *.atmos_{month,daily}_cmip.*`
if ($#radiation_files == 12) then
  foreach INFILE ( $radiation_files )
    set OUTFILE = $refineDiagDir/$INFILE:t:s/.atmos_month_cmip./.atmos_month_refined./
    set OUTFILE = $refineDiagDir/$OUTFILE:t:s/.atmos_daily_cmip./.atmos_daily_refined./
    set TMPFILE = `echo $INFILE:t | sed -e 's/_cmip\./_tmp./'`
    $source_dir/refine_fields.pl radiation_refine $INFILE $TMPFILE
    if ($?) @ refineAtmosErrors++
    if (-e $TMPFILE) then
      if (-e $OUTFILE) then
        # perform append in working directory to avoid long filenames
