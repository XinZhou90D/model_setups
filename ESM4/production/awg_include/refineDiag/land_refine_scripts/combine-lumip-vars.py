...
def addHistory(dst, text):
    """ adds given text to the history attribute of the dst file """
...
parser.add_argument('input', nargs='+',
    help='''input netcdf files for each land use tile, in the following order: psl, pst,
            crp, urb. Urban file may be missing, in which case all urban data are filled
            with missing values.''')
...
dst.createDimension(lu_dim_name,size = (len(lu_names)))

# create variable for land use type names and write the names into it
lu_name_len = max(len(x) for x in lu_names)
namevar = dst.createVariable(lu_dim_name,'i', lu_dim_name)
namevar.long_name = 'Land use type'
namevar[:]= range(0, len(lu_names))
namevar.standard_name= 'area_type'
namevar.flag_values = [0,1,2,3]
namevar.flag_meanings= ' '.join(lu_names)
...
auxVars = set()
auxAttrs = {'bounds','edges','time_avg_info','coordinates'} # attributes that may list auxiliary variables
for var in template.variables.itervalues():
    for attr in auxAttrs:
        if attr not in var.ncattrs(): continue
        for v in re.split(r'[ ,]+',var.getncattr(attr)):
...
for varname in vars:
    ...
    if (template.dimensions[invar.dimensions[0]].isunlimited()) :
        outdim = (invar.dimensions[0],lu_dim_name)+invar.dimensions[1:]
    else:
        outdim = (lu_dim_name,)+invar.dimensions
    ...
    if 'coordinates' in outvar.ncattrs() :
        outvar.coordinates = lu_dim_name+' '+outvar.coordinates
    else:
        outvar.coordinates = lu_dim_name
