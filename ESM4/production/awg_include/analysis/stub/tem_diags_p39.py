...
for outfreq in ('monthly','daily'):
    for varitem in varlist:
        if outfreq == 'monthly':
	    ncoutname=outdir+outfreq+'/atmos_plev39.'+"%04u"%Y1+'01-'+"%04u"%Y2+'12.'+varitem['var']+'.nc'
	    ncout=Dataset(ncoutname,'w',format='NETCDF3_CLASSIC')
            ncout.setncattr('filename','atmos_plev39.'+"%04u"%Y1+'01-'+"%04u"%Y2+'12.'+varitem['var']+'.nc')
            #continue
        else:
            ncoutname=outdir+outfreq+'/atmos_plev39.'+"%04u"%Y1+'0101-'+"%04u"%Y2+'1231.'+varitem['var']+'.nc'
            ncout=Dataset(ncoutname,'w',format='NETCDF3_CLASSIC')
            ncout.setncattr('filename','atmos_plev39.'+"%04u"%Y1+'0101-'+"%04u"%Y2+'1231.'+varitem['var']+'.nc')
            #continue
        #ncout=Dataset(ncoutname,'w',format='NETCDF3_CLASSIC')
	ncout.createDimension('time',None)
	ncout.createDimension('lat',nlat)
	ncout.createDimension('plev39',nlev)
	ncout.createDimension('bnds',2)
	#ncout.setncattr('filename','atmos_plev39.'+"%04u"%Y1+'-'+"%04u"%Y2+'.'+varitem['var']+'.nc')
	ncout.setncattr('title',getattr(ncref,'title'))
	ncout.setncattr('grid_type',getattr(ncref,'grid_type'))
	data={}
	data[varitem['var']]=ncout.createVariable(varitem['var'],ncref.variables['ua'].dtype,('time','plev39','lat'))
	...
	data[varitem['var']].setncattr('dimensions','latitude plev39 time')
	...
	for corvar in ('lat','lat_bnds','plev39','time'):
	...
	data['plev39'].setncattr('comment','plev39 grid defined in Gerber and Manzini (2016)')
...
for iyear in np.arange(Y1,Y2+1):
	...
	for varitem in varlist:
		...
		x=utendepfd
	    #print(x.fill_value)
	    ncout=Dataset(outdir+'monthly/atmos_plev39.'+"%04u"%Y1+'01-'+"%04u"%Y2+'12.'+varitem['var']+'.nc','r+')
	    ...
	    ncout.close()
	    ncout=Dataset(outdir+'daily/atmos_plev39.'+"%04u"%Y1+'0101-'+"%04u"%Y2+'1231.'+varitem['var']+'.nc','r+')
